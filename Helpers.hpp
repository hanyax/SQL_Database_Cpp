...
namespace ECE141 {
  ...
  static std::string noPrimaryKeyProvided = "No Primary Key Provided";
  ...
  public:
    ...
    static std::string makeSpace(int wordSize, int totalSize) {
      std::string result = "";
      for (int i = 0; i < totalSize - wordSize; i++) {
        result += " ";
      }
      return result;
    }
    
    static std::string buildSeperator (int wordLength) {
      std::string result = "+-";
    ...
    static std::string buildAttrView(std::string attr) {
      std::string result = "| ";
      result += attr;
      result += makeSpace(attr.size(), 14);
      return result;
    }
    
    static const int gMultiplier = 37;
    ...
    static std::string readUntil(int index, char* buffer) {
      char result[100];
      int i = 0;
    ...
    static bool isValidName(std::string aName) {
      
      if(!((aName[0] >= 'a' && aName[0] <= 'z') ||
           (aName[0] >= 'A' && aName[0] <= 'Z'))
         ) {
        std::cout << "Name error: " << aName << " is an illegal identifier" << std::endl;
        return false;
      }
      size_t nameLen = aName.length();
      ...
      if(gDictionary.count(aName)) {
          std::cout << "Name error: identifier expected, but keyword " + aName + " found.\n";
          return false;
      }
    ...
    template <class T>
    static int findIndex(std::vector<T> &vect, T member) {
    ...
    template<typename T>
    static bool in_vector(std::vector<T> &v, const T &aValue) {
      return (std::find(v.begin(), v.end(), aValue) != v.end());
    }
